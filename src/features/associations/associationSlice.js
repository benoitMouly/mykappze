...
export const fetchAssociations = createAsyncThunk('associations/fetchAssociations', async (userId) => {
...
export const addAssociation = createAsyncThunk(
  ...
  async ({ userId, associationData }, { rejectWithValue }) => {
    try {
      ...
      const newAssociation = { id: newAssociationRef.id, ...newAssociationSnapshot.data() };

      // Ajoutez l'ID de l'association à la collection "userAssociations" de l'utilisateur
      const userAssociationsRef = collection(db, 'userAssociations');
      await addDoc(userAssociationsRef, { userId, associationId: newAssociation.id });
...
/*
* Update association informations
***    
*/
...
export const updateAssociation = createAsyncThunk(
  ...
  async ({ associationId, associationData }, { rejectWithValue }) => {
    try {
      ...
      const querySnapshot = await getDocs(query(animalsRef, where('associationId', '==', associationId)));
...
export const removeAssociation = createAsyncThunk(
  ...
  async ({ userId, associationId }, { rejectWithValue }) => {
    ...
    try {
      ...
      const userAssociationsQuery = query(collection(db, 'userAssociations'), where('userId', '==', userId), where('associationId', '==', associationId));
...
//       const updatedAssociationData = { id: associationSnapshot.id, ...associationData };
...
export const joinAssociation = createAsyncThunk(
  ...
  async ({ userId, associationId }, { rejectWithValue }) => {
    try {
      ...
      const userAssociationsRef = collection(db, 'userAssociations');
      const querySnapshot = await getDocs(query(userAssociationsRef, where("userId", "==", userId), where("associationId", "==", associationId)));
...
const associationsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateAssociation.fulfilled, (state, action) => {
        ...
        if (state.selectedAssociation && state.selectedAssociation.id === action.payload.associationId) {
          state.selectedAssociatione = { ...state.selectedAssociation, ...action.payload.associationData };
        }
      })
    ...
    builder.addCase(joinAssociation.pending, (state) => {
      state.status = 'loading';
    })
      ...
      builder.addCase(changeUserRole.pending, (state) => {
        ...
        builder.addCase(updateAssociationImage.pending, (state) => {
          state.status = 'loading';
      })
      .addCase(updateAssociationImage.fulfilled, (state, action) => {
          state.status = 'succeeded';
          const { associationId, imageUrl } = action.payload;
          const association = state.data.find((association) => association.id === associationId);
          if (association) {
    ...
    builder.addCase(removeAssociation.fulfilled, (state, action) => {
      state.status = 'succeeded';
      // remove the association from the state
      state.data = state.data.filter(association => association.id !== action.payload.id);
    })
