...
export const fetchMotherById = createAsyncThunk(
  "animals/fetchMotherById",
  async (motherId, { rejectWithValue }) => {
    try {
      const db = getFirestore();
...
export const fetchAnimalsByMother = createAsyncThunk(
  "animals/fetchAnimalsByMother",
  async (motherId) => {
    const db = getFirestore();
    const q = query(
      collection(db, "animals"),
      where("motherAppId", "==", motherId)
    );
    const querySnapshot = await getDocs(q);
    const animalsData = [];
    querySnapshot.forEach((doc) => {
...
export const fetchAnimalsByCitySector = createAsyncThunk(
  "animals/fetchAnimalsByCitySector",
  async (citySectorId) => {
    const db = getFirestore();
    const q = query(
      collection(db, "animals"),
  ...
  async ({ animalId, documents }, thunkAPI) => {
    try {
      ...
      const uploadPromises = documents.map((doc) =>
        thunkAPI.dispatch(uploadFile(doc))
      );
      ...
      await thunkAPI.dispatch(
        addDocumentToAnimal({ animalId, documents: uploadedDocuments })
      );
...
export const uploadSingleFile = async (file, animalName) => {
  ...
  try {
    ...
    console.log("euh, grosse erreur là -> ", error);
...
export const deleteDocumentFromAnimal = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    try {
      ...
      const animalData = {
        ...animal,
        documents: animal.documents.filter(
          (singleDocument) => singleDocument.name !== documentName
        ),
      };
...
//             // Récupérer les noms des fichiers associés à l'animal (ajustez selon la structure de vos données)
//             const animalDoc = await getDoc(animalRef);
//             const animalData = animalDoc.data();
//             const fileNames = animalData.documents; // Supposons que "documents" est un tableau de noms de fichiers

//             // Supprimer chaque fichier de Google Storage
//             for (let fileName of fileNames) {
//                 await dispatch(deleteFileFromStorage(fileName.name));
//             }
...
export const deleteAnimal = createAsyncThunk(
  ...
  async (animalId, { rejectWithValue, dispatch }) => {
    try {
      ...
      if (animalData && animalData.image && animalData.image.name) {
        console.log("NAME :", animalData.image.name);
        dispatch(deleteImageFromStorage(animalData.image.name));
      }
    ...
    builder
      ...
      .addCase(updateAnimal.fulfilled, (state, action) => {
        ...
        if (
          ...
          state.selectedAnimal.id === action.payload.animalId
        ) {
          state.selectedAnimal = {
            ...state.selectedAnimal,
            ...action.payload.animalData,
          };
        }
      })
      ...
      .addCase(deleteFileFromStorage.fulfilled, (state, action) => {
        ...
        state.selectedAnimal.documents = state.selectedAnimal.documents.filter(
      ...
      .addCase(deleteImageFromStorage.pending, (state) => {
        state.status = "loading";
      })
